...
(function () {
	...
	var	AudioHelper = function (component) {

		return {

			load: function () {

				component.state.audio.load ();
				component.state.muted = this.isMuted ();
				this.setVolume (component.config.volume);
				component.state.audio.addEventListener ('timeupdate', function () {
			...
			play: function () {
				...
				component.state.audio.play ();		
			},

			stop: function () {

				component.state.audio.pause ();
			},

			getVolume: function () {

				return Math.floor (10 * component.state.audio.volume);
			},

			setVolume: function (volume) {

				if (component.componentHelper.isDefined (volume)) {
					     if (volume <  0)  { component.state.audio.volume = 0; }
					else if (volume >= 10) { component.state.audio.volume = 1;  }
					else                   { component.state.audio.volume = volume / 10; }				
				}
			},

			isMuted: function () {

				return component.state.audio.muted;
			},

			setMuted: function (muted) {

				component.state.audio.muted = muted;
				component.state.muted       = muted;
			},

			getTime: function () {

				return component.state.audio.currentTime;
			},
			...
			setTime: function (time) {
				...
				if (component.componentHelper.isDefined (time)) {
					...
					if (targetTime < component.state.audio.currentTime) {
						component.state.audio.currentTime = targetTime;
			...
			getLength: function () {
				...
				return component.state.audio.duration;
			},

			isStopped: function () {

				return component.state.audio.paused;
			},
	...
	var EventHelper = function (component) {
		...
		return {
			...
			snapshot: function () {
				...
				var audioHelper = component.audioHelper;
				return {
					time    : audioHelper.getTime (),
					length  : audioHelper.getLength (),
					stopped : audioHelper.isStopped (),
					volume  : audioHelper.getVolume (),
	...
	Polymer ('wc-audio', {
		...
		init: function () {
			...
			this.audioHelper     = AudioHelper (this);
		...
		backward: function (delta) {
			...
			this.eventHelper.before ();
			var time = this.audioHelper.getTime ();
			this.audioHelper.setTime (time - delta);
		...
		mute: function (muted) {
			...
			if (!this.componentHelper.isDefined (muted)) {
				muted = this.audioHelper.isMuted ();
			}
			this.eventHelper.before ();
			this.audioHelper.setMuted (!muted);
			if (!muted && this.audioHelper.getVolume () == 0) {
				this.audioHelper.setVolume (this.config.volume);
			}
		...
		onTime: function (event) {
			...
			this.onGo (event);
			if ((event.data.change.time > 0) && this.audioHelper.isStopped ()) {
				this.audioHelper.play ();
			}				
		},
		...
		doBackward: function () {
			...
			var delta = this.config.deltas.audio;
			this.backward (delta);	
		},
		...
		doGoEnd: function () {
			...
			var length = this.audioHelper.getLength ();
			this.stop ();
			this.go (length);			
		},
		...
		doVolumeUp: function (delta) {
			...
			var volume = this.audioHelper.getVolume () + delta;
			this.volume (volume);
		},
